...
interface CodeEditorProps {
  ...
  errorLines?: number[];            // NEW: multi-line highlight
...
export const CodeEditor = forwardRef<CodeEditorHandle, CodeEditorProps>(
  function CodeEditor(
    { value, onChange, language, errorLine, errorLines = [], onSelectionChange },
    ref
  ) {
    ...
    // Apply decorations whenever errorLine or errorLines changes
    useEffect(() => {
      const editor = editorRef.current;
      const monaco = monacoRef.current;
      if (!editor || !monaco) return;

      // Merge single errorLine + errorLines array, deduplicate
      const allLines = Array.from(
        new Set([
          ...(errorLine && errorLine > 0 ? [errorLine] : []),
          ...errorLines.filter((l) => l > 0),
        ])
      );
